...
def decode_from_file(estimator, filename, decode_hp, decode_to_file=None,
  ...
  sorted_inputs, sorted_keys, sorted_targets = _get_sorted_inputs(
      filename, decode_hp.shards, decode_hp.delimiter, decode_targets_file)
  ...
  tf.logging.info("Used time:" + repr(used_time))
  tf.logging.info("Average time:" + repr(used_time / len(sorted_inputs)))
...
def _decode_filename(base_filename, problem_name, decode_hp):
  return ("{base}.{model}.{hp}.{problem}"
          ".beam{beam}.alpha{alpha}.decodes{dedup}").format(
      base=base_filename,
      model=FLAGS.model,
      ...
      dedup=".dedup" if FLAGS.dedup else "")
...
def _decode_batch_input_fn(problem_id, num_decode_batches, sorted_inputs,
  ...
  if sorted_targets is None:
    ...
    for b in range(num_decode_batches):
      ...
      batch_length = 0
      batch_inputs = []
      batch_targets_length = 0
      batch_targets = []
      for inputs, targets in zip(
          sorted_inputs[b * batch_size:(b + 1) * batch_size],
          sorted_targets[b * batch_size:(b + 1) * batch_size]
      ):
        input_ids = vocabulary.encode(inputs)
        targets_ids = targets_vocab.encode(targets)
        if max_input_size > 0:
          # Subtract 1 for the EOS_ID.
          input_ids = input_ids[:max_input_size - 1]
        input_ids.append(text_encoder.EOS_ID)
        targets_ids.append(text_encoder.EOS_ID)
        batch_inputs.append(input_ids)
        batch_targets.append(targets_ids)
        if len(input_ids) > batch_length:
          batch_length = len(input_ids)
        if len(targets_ids) > batch_targets_length:
          batch_targets_length = len(targets_ids)
      final_batch_inputs = []
      for input_ids in batch_inputs:
        assert len(input_ids) <= batch_length
        x = input_ids + [0] * (batch_length - len(input_ids))
        final_batch_inputs.append(x)
      final_batch_targets = []
      for targets_ids in batch_targets:
        assert len(targets_ids) <= batch_targets_length
        x = targets_ids + [0] * (batch_targets_length - len(targets_ids))
        final_batch_targets.append(x)
      yield {
        "inputs": np.array(final_batch_inputs).astype(np.int32),
        "targets": np.array(final_batch_targets).astype(np.int32),
        "problem_choice": np.array(problem_id).astype(np.int32),
      }
...
def _get_sorted_inputs(filename, num_shards=1, delimiter="\n",
  ...
  if targets_filename is not None:
    with tf.gfile.Open(targets_filename) as f:
      text = f.read()
      records = text.split(delimiter)
...
def _decode_input_tensor_to_features_dict(feature_map, hparams):
  ...
  Returns:
    a features dictionary, as expected by the decoder.
  """
  inputs = tf.convert_to_tensor(feature_map["inputs"]) # bs * max_len
  
  targets = feature_map.get("targets")
  ...
  input_space_id, target_space_id, x, y = input_fn_builder.cond_on_index(
      input_fn, feature_map["problem_choice"], len(hparams.problems) - 1)
