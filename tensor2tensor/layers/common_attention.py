...
def dot_product_attention(q,
  ...
  with tf.variable_scope(
    ...
    if bias is not None:
      logits += bias
    # logits = tf.Print(logits, [tf.shape(logits), tf.shape(bias)],
    #                   summarize=100,
    #                   message="logits & bias shape")
    ...
    # layer_id = None
    # attention_type = None
    # weights_names = weights.name.split('/')
    # if 'decoder' in weights_names:
    #   for name in weights_names:
    #     if "layer" in name:
    #       layer_id = name
    #     if name == 'self_attention':
    #       attention_type = 'self_att'
    #     if name == 'encdec_attention':
    #       attention_type = 'encdec_att'
    #     if name == 'positional_attention':
    #       attention_type = 'pos_att'
    #
    #
    #   weights = dump_hidden(weights,
    #
    #                         '.'.join([layer_id, attention_type, "attention"]),
    #
    #                         dump_measure="attention")
    if return_attention_weight:
      return tf.matmul(weights, v), weights_without_dropout
  ...
  with tf.variable_scope(
    ...
    if callable(attention_type):  # Generic way to extend multihead_attention
      ...
      x = dot_product_attention(q, k, v, bias, dropout_rate, image_shapes,
                                return_attention_weight=kwargs.get(
                                    "return_attention_weight", False))
      if isinstance(x, tuple):
